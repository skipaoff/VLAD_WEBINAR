# Дизайн-паттерн «Health Portal»

Это тот самый бесплатный паттерн MotionSites, по которому собран наш сайт клиники.
Текст лежит здесь целиком, чтобы подключать MCP-коннектор было не нужно: открыл
проект — и всё уже под рукой.

Собирай строго по спецификации ниже: те же классы, те же хуки, те же анимации,
мозаика MaskedCard, фиксированный navbar. Прелоадер (splash screen) из паттерна
**не делай** — сайт должен открываться сразу. Картинки из блока IMAGE URLS замени
на фотографии клиники из `klient/foto/`, тексты — на данные из анкеты.

Оригинал и вся библиотека: https://motionsites.ai/?prompt=health-portal
